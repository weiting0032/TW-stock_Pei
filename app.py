...
st.set_page_config(page_title="台股戰情指揮中心 V14.1 (修正版)", layout="wide", page_icon="📈")
...
def get_market_data_finmind():
    """修正版：獲取全市場清單與財務指標 (增加日期容錯)"""
    fm_url = "https://api.finmindtrade.com/api/v4/data"
    # 若有 API Token 建議加入，沒有則使用匿名限制
    ...
    try:
        ...
        # 2. 獲取本益比/淨值比 (嘗試多個日期直到抓到資料)
        per_df = pd.DataFrame()
        for i in range(1, 10): # 往回找 10 天
            search_date = (datetime.now() - timedelta(days=i)).strftime('%Y-%m-%d')
            per_params = {"dataset": "TaiwanStockPER", "start_date": search_date, "token": api_token}
        ...
        if per_df.empty:
            st.warning("無法取得近期財務指標資料，快篩功能可能不完整。")
            # 建立空的資料結構避免後續報錯
...
def get_strategy_suggestion(df):
    if df is None or df.empty or len(df) < 30: 
        return ("計算中", "#9e9e9e", "<span>數據載入中或樣本不足...</span>", "")
    last = df.iloc[-1]
    prev = df.iloc[-2]
    ...
    if rsi < 30: return ("超賣區", "#d32f2f", "🔥 股價進入超賣區，隨時可能反彈", f"RSI: {rsi:.1f}")
    if close < lower and hist > prev['Hist']: return ("抄底訊號", "#2e7d32", "🚀 觸及布林下軌且動能轉強", "技術面抄底")
    if rsi > 70: return ("超買區", "#ef6c00", "⚠️ 漲幅過大，注意調節風險", f"RSI: {rsi:.1f}")
    if close > last['SMA20']: return ("多頭持有", "#1976d2", "📈 站於均線之上，動能健康", "多頭趨勢")
    return ("整理中", "#757575", "☕ 市場觀望中，無明確趨勢", f"RSI: {rsi:.1f}")
...
if st.session_state.menu == "portfolio":
    st.markdown('<div class="function-title">🚀 庫存動態監控</div>', unsafe_allow_html=True)
    ...
    if not portfolio.empty:
        ...
        for _, r in portfolio.iterrows():
            ...
            if h_df is not None:
                ...
                m_info = MARKET_MAP.get(r['Symbol'], {'名稱': r['Name'], '產業': '-', 'PE': 0, 'PB': 0})
        ...
        for i, item in enumerate(details):
            ...
            with cols[i % 3]:
                ...
                """, unsafe_allow_html=True)
                if st.button(f"技術分析 {r['Symbol']}", key=f"p_{r['Symbol']}"):
    ...
    if 'scan_results' in st.session_state:
        ...
        if not df_res.empty:
            st.success(f"符合標的：{len(df_res)} 筆 (僅顯示前 30 筆)")
            ...
            for i, (idx, row) in enumerate(df_res.head(30).iterrows()):
                with sc_cols[i % 3]:
                    ...
                    <div class="stock-card">
                        ...
                        <div style="margin:5px 0;">PE: <b>{row['PE']:.1f}</b> | PB: <b>{row['PB']:.1f}</b></div>
                    </div>
                    """, unsafe_allow_html=True)
                    if st.button(f"詳細診斷 {row['代碼']}", key=f"sc_{row['代碼']}"):
    ...
    selection = st.selectbox("請輸入代碼或名稱搜尋", options=["請選擇..."] + STOCK_OPTIONS)
